...
let selectedTowerForUpgrade = null;  // Track which tower is selected for upgrade
  ...
  function setup() {
    ...
    mainCoverImg = createGraphics(800, 600);
    
    // Draw gradient background
    for(let y = 0; y < 600; y++) {
      let inter = map(y, 0, 600, 0, 1);
      let c = lerpColor(color(50, 30, 10), color(10, 10, 30), inter);
      mainCoverImg.stroke(c);
      mainCoverImg.line(0, y, 800, y);
    }
    
    // Draw decorative elements
    mainCoverImg.noFill();
    mainCoverImg.stroke(255, 165, 0, 100);
    mainCoverImg.strokeWeight(2);
    for(let i = 0; i < 5; i++) {
      mainCoverImg.ellipse(400, 250, 300 + i*30, 300 + i*30);
    }
    
    // Draw Bitcoin symbol
    mainCoverImg.push();
    mainCoverImg.translate(400, 250);
    mainCoverImg.fill(255, 165, 0);
    mainCoverImg.stroke(255, 140, 0);
    mainCoverImg.strokeWeight(5);
    mainCoverImg.ellipse(0, 0, 180, 180);
    mainCoverImg.fill(50, 30, 10);
    mainCoverImg.textSize(110);
    mainCoverImg.textAlign(CENTER, CENTER);
    mainCoverImg.text("₿", 0, 10);
    mainCoverImg.pop();
    
    // Add title with shadow effect
    mainCoverImg.textAlign(CENTER, CENTER);
    mainCoverImg.textSize(50);
    mainCoverImg.fill(0, 0, 0, 80);
    mainCoverImg.text("MYSTERIES OF THE", 403, 103);
    mainCoverImg.text("BITCOIN CITADEL", 403, 403);
    
    mainCoverImg.fill(255, 215, 0);
    mainCoverImg.text("MYSTERIES OF THE", 400, 100);
    mainCoverImg.text("BITCOIN CITADEL", 400, 400);
    
    // Add subtitle with shadow
    mainCoverImg.fill(0, 0, 0, 80);
    mainCoverImg.textSize(24);
    mainCoverImg.text("Coming Soon - April 7th", 403, 463);
    
    mainCoverImg.fill(200, 200, 200);
    mainCoverImg.text("Coming Soon - April 7th", 400, 460);
    ...
    // Remove the duplicate canvas creation and touch handler
    // createCanvas(800, 600).touchStarted(handleTouchStart);
  }
  ...
  function draw() {
    if (gameState === 'playing') {
      ...
      // After delay, show promotion screen instead of end screen
      gameState = 'promotion';
      promotionScreenDelay = frameCount + 900; // 15 seconds at 60fps
      promotionAlpha = 0;
      promotionFlickerTimer = 0;
      ...
      // After promotion delay, show end screen
      if (frameCount >= promotionScreenDelay) {
        gameState = lives <= 0 ? 'gameover_end' : 'victory_end';
        endScreenAlpha = 0;
      }
    } else if (gameState === 'gameover_end' || gameState === 'victory_end') {
      // Continue drawing game state in background
      image(bg, 0, 0);
      drawPath();
      drawTowerSpots();
      ...
      textAlign(CENTER);
      textSize(64);
      let titleColor = gameState === 'victory_end' ? color(255, 215, 0) : color(255, 50, 50);
      
      // Glow effect
      for (let i = 4; i > 0; i--) {
        let alpha = map(i, 4, 0, 50, 150);
        fill(titleColor.levels[0], titleColor.levels[1], titleColor.levels[2], alpha);
        text(gameState === 'victory_end' ? "Victory!" : "Game Over", width/2, centerY + i);
      }
      fill(titleColor);
      text(gameState === 'victory_end' ? "Victory!" : "Game Over", width/2, centerY);
  ...
  function mousePressed() {
    ...
    if (showLeaderboardSubmitDialog) {
      ...
      // Check if click is outside dialog (to dismiss it)
      ...
      if (submitButtonHovered) {
        ...
        // Prompt for nickname
        let nickname = prompt("Enter your nickname (max 8 characters):", leaderboardNickname);
        if (nickname !== null) {
          leaderboardNickname = nickname.substring(0, 8); // Limit to 8 chars
        }
        return;
    ...
    if (showLeaderboard) {
      ...
      // Check if click is outside leaderboard (to dismiss it)
    ...
    // Handle promotion screen buttons
    if (gameState === 'promotion') {
      ...
      promotionButtons.forEach(button => {
        if (button.hovered) {
          buttonClicked = true;
          if (button.action === 'youtube') {
            window.open(button.url, '_blank');
          } else if (button.action === 'twitter') {
            window.open(button.url, '_blank');
          } else if (button.action === 'skip') {
            // Skip promotion and go to end screen
            gameState = lives <= 0 ? 'gameover_end' : 'victory_end';
            endScreenAlpha = 0;
          }
        }
    ...
    if ((gameState === 'gameover_end' || gameState === 'victory_end') && frameCount >= endScreenDelay) {
      // Check if we're clicking on any button
      let buttonClicked = false;
      let buttonY = height / 2 - 200 + 260; // Same calculation as in draw()
      let buttonSpacing = 50;
      
      endScreenButtons.forEach((button, i) => {
        let y = buttonY + i * buttonSpacing;
        let isHovered = mouseX > width/2 - 150 && mouseX < width/2 + 150 &&
                      mouseY > y - 25 && mouseY < y + 25;
        ...
        if (isHovered) {
          ...
          if (button.action === 'restart') {
            ...
            let text = gameState === 'victory_end' ? 
      ...
      // If we clicked outside any button, reset all button states
      if (!buttonClicked) {
...
function drawLeaderboard() {
  ...
  if (leaderboardData.length === 0) {
    ...
    for (let i = 0; i < Math.min(leaderboardData.length, 10); i++) {
      ...
      // Nickname (truncated if too long)
      textAlign(LEFT);
      let displayName = entry.nickname.length > 8 ? entry.nickname.substring(0, 8) + "..." : entry.nickname;
      text(displayName, boardX + 120, y);
      
      // Score and Bitcoiner Rank (with fewer decimal places)
      textAlign(RIGHT);
      text(`${entry.score.toFixed(4)} (${entry.rank})`, boardX + boardWidth - 50, y);
    }
  }
...
function submitScore() {
  ...
  // Ensure nickname is limited to 8 characters
  const limitedNickname = leaderboardNickname.substring(0, 8);
...
//   if (touches.length > 0) {
//     // Map the touch position to mouseX and mouseY
//     mouseX = touches[0].x;
//     mouseY = touches[0].y;
//     
//     // Call the existing mousePressed function
//     mousePressed();
//     
//     // Prevent default
//     return false;
...
// Add touchEnded function
function touchEnded() {
  // Prevent default behavior
  return false;
  }

// Promotion screen variables
let mainCoverImg;
let promotionScreenDelay = 0;
let promotionAlpha = 0;
let promotionFlickerTimer = 0;
let promotionButtons = [
  { text: 'YouTube', action: 'youtube', url: 'https://www.youtube.com/channel/UCE01sKwTZj--1QS11getdzQ', hovered: false },
  { text: 'X (Twitter)', action: 'twitter', url: 'https://x.com/paperstreet_inc', hovered: false },
  { text: 'Skip', action: 'skip', hovered: false }
];

// Add a flag to track if we've attempted to load the image
let attemptedImageLoad = false;

// Function to draw the promotion screen
function drawPromotionScreen() {
  // Try to load the image once if not already attempted
  if (!attemptedImageLoad && !mainCoverImg) {
    attemptedImageLoad = true;
    loadImage('Assets/Main Cover.png', 
      img => { mainCoverImg = img; console.log('Image loaded successfully'); },
      err => { console.error('Failed to load image', err); }
    );
  }
  
  // Fade in effect
  promotionAlpha = min(promotionAlpha + 3, 255);
  ...
  // Flicker effect (every 120 frames / 2 seconds)
  promotionFlickerTimer++;
  let displayImage = true;
  
  // Create flicker effect at specific times
  if (promotionFlickerTimer > 120 && promotionFlickerTimer < 125 || 
      promotionFlickerTimer > 300 && promotionFlickerTimer < 303) {
    displayImage = false;
  }
  
  // If mainCoverImg is defined and image is loaded
  if (displayImage && mainCoverImg && mainCoverImg.width > 0) {
    // Display the Main Cover image with proper scaling
    push();
    imageMode(CENTER);
    
    // Calculate scaling to fit in the screen while maintaining aspect ratio
    let imgRatio = mainCoverImg.width / mainCoverImg.height;
    let imgWidth = width * 0.8; // Use 80% of screen width
    let imgHeight = imgWidth / imgRatio;
    
    // If too tall, scale based on height
    if (imgHeight > height * 0.7) {
      imgHeight = height * 0.7;
      imgWidth = imgHeight * imgRatio;
    }
    
    // Apply fade-in effect
    tint(255, promotionAlpha);
    image(mainCoverImg, width/2, height/2 - 50, imgWidth, imgHeight);
    pop();
  } else {
    // Fallback if image isn't loaded
    push();
    rectMode(CENTER);
    fill(40, 45, 60, promotionAlpha);
    stroke(255, 165, 0, promotionAlpha);
    strokeWeight(2);
    rect(width/2, height/2 - 50, width * 0.8, height * 0.5, 10);
    
    if (!mainCoverImg || mainCoverImg.width === 0) {
      fill(255, promotionAlpha);
      textAlign(CENTER);
      textSize(20);
      text("Image Loading...", width/2, height/2 - 50);
    }
    pop();
  }
  ...
  textAlign(CENTER);
  textSize(32);
  fill(255, 215, 0, promotionAlpha);
  text("Mysteries of the Bitcoin Citadel", width/2, height - 180);
  textSize(18);
  fill(200, 200, 200, promotionAlpha);
  text("Coming Soon!", width/2, height - 150);
  pop();
  ...
  promotionButtons.forEach((button, i) => {
    let x = width/2 + (i-1) * buttonSpacing;
    button.hovered = mouseX > x - buttonWidth/2 && mouseX < x + buttonWidth/2 &&
                    mouseY > buttonY - buttonHeight/2 && mouseY < buttonY + buttonHeight/2;
    
    // Button background
    fill(button.hovered ? color(255, 165, 0, promotionAlpha) : color(40, 45, 60, promotionAlpha));
    stroke(255, 165, 0, promotionAlpha);
    strokeWeight(2);
    rect(x - buttonWidth/2, buttonY - buttonHeight/2, buttonWidth, buttonHeight, 8);
    
    // Button text
    fill(button.hovered ? color(0, promotionAlpha) : color(255, promotionAlpha));
    noStroke();
    text(button.text, x, buttonY);
  });
  pop();
  }